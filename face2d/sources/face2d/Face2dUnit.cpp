...
bool Face2dUnit::initWithConfig(CCDictionary* config,Face2dNode* orginNode)
{
    ...
    for (int i = 0; i < triangle->count(); ++i)
    {
        ...
        Face2dTriangle tria = {(GLushort)i_0,(GLushort)i_1,(GLushort)i_2};
        m_all_triangle.push_back(tria);
    }
    return true;
}

void Face2dUnit::draw()
{
...
#define Face2d_Color_Size sizeof(Face2d_Color)
    ...
    // vertex
    glVertexAttribPointer(kCCVertexAttrib_Position, 3, GL_FLOAT, GL_FALSE, Face2d_Vertex_Size, (void*)m_now_vertex.data());
    glVertexAttribPointer(kCCVertexAttrib_TexCoords, 2, GL_FLOAT, GL_FALSE, Face2d_UV_Size, (void*)m_all_uv.data());
    glVertexAttribPointer(kCCVertexAttrib_Color, 4, GL_UNSIGNED_BYTE, GL_TRUE, Face2d_Color_Size, (void*)m_now_color.data());
    ...
    for(vector<Face2dTriangle>::iterator iter = m_all_triangle.begin();
        iter != m_all_triangle.end(); ++iter)
    {
        Face2dTriangle& points = *iter;
        float a,b;
