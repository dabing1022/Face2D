    ...
    int       nCmdShow)
{
    ...
    return Face2dPlayer::createAndRun();
}

int Face2dPlayer::createAndRun(void)
{
    Face2dPlayer *host = Face2dPlayer::sharedInstance();
    int ret = host->run();
    Face2dPlayer::purgeSharedInstance();
    return ret;
}

Face2dPlayer *Face2dPlayer::s_sharedInstance = NULL;

Face2dPlayer *Face2dPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new Face2dPlayer();
    }
    return s_sharedInstance;
}

void Face2dPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
Face2dPlayer::Face2dPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int Face2dPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "face2d-player");
...
void Face2dPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void Face2dPlayer::createViewMenu(void)
{
...
void Face2dPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void Face2dPlayer::updateWindowTitle(void)
{
}

void Face2dPlayer::relaunch(void)
{
...
void Face2dPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void Face2dPlayer::onFileOpenProject(void)
{
...
void Face2dPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"Face2dPlayer");
...
void Face2dPlayer::onFileProjectConfig(void)
{
...
void Face2dPlayer::onFileRelaunch(void)
{
    relaunch();
}

void Face2dPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void Face2dPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void Face2dPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void Face2dPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void Face2dPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT Face2dPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    Face2dPlayer *host = Face2dPlayer::sharedInstance();
...
INT_PTR CALLBACK Face2dPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
