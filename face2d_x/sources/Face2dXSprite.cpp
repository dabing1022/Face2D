...
Face2dXSprite* Face2dXSprite::create(const char* pictruePath, const char* configPath)
{
    Face2dXSprite *pobSprite = new Face2dXSprite();
...
bool Face2dXSprite::initWithFile(const char* pictruePath, const char* configPath)
{
...
bool Face2dXSprite::initWithConfig( const char* configPath)
{
...
#define Face2dX_Color_Size sizeof(Face2dX_Color)
    ...
    // vertex
    glVertexAttribPointer(kCCVertexAttrib_Position, 3, GL_FLOAT, GL_FALSE, Face2dX_Vertex_Size, (void*)m_nowVertex.data());
    
    // texCoods
    glVertexAttribPointer(kCCVertexAttrib_TexCoords, 2, GL_FLOAT, GL_FALSE, Face2dX_UV_Size, (void*)m_orginUV.data());
    
    // color
    glVertexAttribPointer(kCCVertexAttrib_Color, 4, GL_UNSIGNED_BYTE, GL_TRUE, Face2dX_Color_Size, (void*)m_orginColor.data());
...
void Face2dXSprite::runAnimation(string name)
{
    map<string, Face2dX_Anims>::iterator iter = m_all_anims.find(name);
    ...
    for (Face2dX_Anims::iterator iter2 = m_curAnims->begin();
...
void Face2dXSprite::animationUpdate(float time)
{
    m_animNowTime += time;
...
//    CCLOG("=========");
    for (Face2dX_Anims::iterator anim = m_curAnims->begin();
         anim != m_curAnims->end(); ++anim)
    {
        float p = (m_animNowTime - anim->delay)/(m_animTime - anim->delay);
        for (Face2dX_Unit_Anims::iterator unit_anim = anim->anim->begin();
             unit_anim != anim->anim->end(); ++unit_anim)
        {
            ...
            if (p >= unit_anim->start_perc && p <= unit_anim->end_perc)
            {
                ...
                {
                    case Face2dX_Animation_Position:
                    {
                        map<int,Face2dX_Unit_Anim_Pos>& position = m_all_point_move[unit_anim->anim_id];
                        for (map<int,Face2dX_Unit_Anim_Pos>::iterator pos = position.begin();
                             pos != position.end(); ++pos)
                        {
                            ...
                            Face2dX_TriangleList& tlist = m_all_units[anim->unit_name];
                            for (Face2dX_TriangleList::iterator iter3 = tlist.begin(); iter3 != tlist.end(); ++iter3)
                            {
                                Face2dX_Triangle& trian = *iter3;
                                for (int i = 0; i < 3; ++i)
                                {
                                    if (trian.orginPoint[i] == pos_id)
                                    {
                                        Face2dX_Vertex value = {x,y,0};
...
void Face2dXSprite::convertDictToConfig(CCDictionary* config)
{
    ...
    for (int i = 0; i < unit_names->count(); ++i)
    {
        ...
        Face2dX_TriangleList& triangle_list = m_all_units[unit_name->m_sString];
        for (int k = 0; k < triangle->count(); ++k)
        {
            ...
            Face2dX_Triangle tconfig;
            
            for (int m = 0; m < 3; ++m)
            {
                CCArray* origin_points = (CCArray*)point->objectAtIndex(point_index[m]);
                Face2dX_UV uv = {floatAtArray(0,origin_points),floatAtArray(1,origin_points)};
                Face2dX_Vertex vertex = {floatAtArray(2,origin_points),floatAtArray(3,origin_points),0};
...
//        m_all_units[unit_name->m_sString] = triangle_list;
        ...
        if(animation)
        {
            map<string,Face2dX_Unit_Anims>& anim2 = m_all_unitanims[unit_name->m_sString];
            ...
            for (int k = 0; k < unit_anim_names->count(); ++k)
            {
                ...
                Face2dX_Unit_Anims& unit_anims2 = anim2[unit_anim_name->m_sString];
                for (int m = 0; m < unit_anims->count(); ++m)
                {
                    CCDictionary* anim_item = (CCDictionary*)unit_anims->objectAtIndex(m);
                    Face2dX_Unit_Anim anim;
                    anim.type = (Face2dX_Animation_Type)anim_item->valueForKey("type")->intValue();
                    ...
                    map<int,Face2dX_Unit_Anim_Pos> anim_pos;
    ...
    for (int i = 0;animation_names != NULL && i < animation_names->count(); ++i)
    {
        ...
        Face2dX_Anims& anim_list = m_all_anims[animation_name->m_sString];
